...
export const enterpriseConnectors: EnterpriseConnector[] = [
  ...
  },
  {
    ...
    requiredPermissions: ["Restricted API key with explicit Twilio read/list permissions", "Tenant-scoped diagnostic installation approval"],
    supportedActions: [
      {
        actionKey: "twilio_diagnostic_read",
        capabilityKey: "twilio.diagnostic.read",
        label: "Run one approved Preview diagnostic read",
        type: "read",
        risk: "low",
        approvalRequired: true,
        safeAutoEligible: false,
    ...
    readCapabilities: ["account identity/status", "owned-number metadata", "inbound webhook readiness", "toll-free verification state", "message delivery metadata without bodies", "error metadata"],
    writeCapabilities: ["blocked SMS", "blocked calls", "blocked number/webhook/account/key mutation"],
    humanApprovalRequirements: ["Exact single-use CEO authorization", "Preview-only environment", "Restricted read permission verification"],
    safeAutoEligibility: "internal_only",
    rateLimits: "Three approved Preview diagnostic attempts per tenant/minute; provider 429 fails closed",
    usageQuotas: "Exactly one provider GET per single-use authorization",
    estimatedCost: "Paid provider; cost controls required before activation",
    healthStatus: "readiness_only",
    lastSuccessfulSync: null,
    lastFailedSync: null,
    retryPolicy: "No automatic retry; record a normalized diagnostic exception.",
    timeoutPolicy: "Five-second provider timeout; fail closed.",
    circuitBreakerState: "not_applicable",
    loggingConfiguration: "No credentials, authorization headers, message bodies, or full phone numbers",
    auditConfiguration: "Audit preflight, credential-reference resolution, provider attempt, completion/failure, and single-use authorization consumption",
    riskLevel: "high",
    environmentSupport: ["development", "preview", "production"],
    featureFlags: ["connector_communication", "ueip_gateway_enforcement"],
    dependencies: ["safe_auto_internal", "ueip_runtime_gateway", "read_only_business_connections"],
    owner: "Revenue Operations",
    credentialReference: "TWILIO_DIAGNOSTIC_RESTRICTED_API_KEY_REFERENCE",
