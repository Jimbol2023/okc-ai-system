...
import {
  assertR52LeadDetailManualRevenueSopScopeInvariants,
  createR52LeadDetailManualRevenueSopScopeContract,
  type R52LeadDetailManualRevenueSopScopeInput,
  type R52LeadDetailManualRevenueSopScopeResult,
} from "./r52-lead-detail-manual-revenue-sop-scope-contract";

const readyInput: R52LeadDetailManualRevenueSopScopeInput = {
...
function assertNoExecution(result: R52LeadDetailManualRevenueSopScopeResult) {
...
describe("R52 lead detail manual revenue SOP scope contract", () => {
  it("missing default input fails closed", () => {
    const result = createR52LeadDetailManualRevenueSopScopeContract();
  ...
  it("selects the top of lead detail page as the first placement", () => {
    const result = createR52LeadDetailManualRevenueSopScopeContract(readyInput);

    assert.equal(result.scopeStatus, "lead_detail_scope_ready");
    assert.equal(result.firstLeadDetailPlacement, "top_of_lead_detail_page");
    assert.equal(result.implementationBoundary.allowedFile, "components/dashboard/lead-detail-client.tsx");
    assert.equal(result.implementationBoundary.allowedPlacement, "top_of_lead_detail_page");
    assertNoExecution(result);
  });

  it("defines exact read-only lead detail content scope for R52M", () => {
    const result = createR52LeadDetailManualRevenueSopScopeContract(readyInput);
  ...
  it("dangerous exclusions include sending automation provider auto-contact and buyer auto-share", () => {
    const result = createR52LeadDetailManualRevenueSopScopeContract(readyInput);
  ...
  it("runtime provider sending automation polling persistence mutation and permission requests are blocked", () => {
    ...
    assert.equal(result.scopeStatus, "lead_detail_scope_blocked");
  ...
  it("validation plan checks build diff isolation automation-agent providers polling and manual reviewed copy", () => {
    const result = createR52LeadDetailManualRevenueSopScopeContract(readyInput);
    const checks = result.r52mValidationPlan.map((item) => item.check).join(" ");
  ...
  it("bounded operator notes are enforced", () => {
    const notes = Array.from({ length: 80 }, (_, index) => `lead_detail_scope_note_${index}_${"x".repeat(220)}`);
    const result = createR52LeadDetailManualRevenueSopScopeContract({
      ...readyInput,
