...
function getTimeoutMs(env: NodeJS.ProcessEnv) {
  const configured = Number(env.PROPERTY_GEOCODE_PREVIEW_TIMEOUT_MS);
  return Number.isFinite(configured) && configured > 0 ? Math.min(configured, 5000) : 2500;
}
...
export async function runPreviewOnlyGeocodeCertification(input: RunPreviewOnlyGeocodeInput): Promise<PreviewOnlyGeocodeResult> {
  ...
  if (response === "timeout") {
    return blocked("provider_timeout", `Geocoding provider did not respond within ${timeoutMs}ms.`, true);
  }
