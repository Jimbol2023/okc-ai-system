import {
  createR89RevenueBottleneckResolutionReadonlyUiScopeContract,
  summarizeR89RevenueBottleneckResolutionReadonlyUiScope,
} from "./r89-revenue-bottleneck-resolution-readonly-ui-scope-contract";

const reviewedInput = {
  authorizedSurfacesReviewed: true,
  bottleneckDiagnosisWordingReviewed: true,
  throughputRecoveryWordingReviewed: true,
  revenueDelayWordingReviewed: true,
  remediationWordingReviewed: true,
  workflowBlockageWordingReviewed: true,
  assignmentClosingBlockageWordingReviewed: true,
...
describe("R89C revenue bottleneck resolution readonly UI scope contract", () => {
  it("defaults to operator review required", () => {
    const result = createR89RevenueBottleneckResolutionReadonlyUiScopeContract();
    expect(result.status).toBe("operator_review_required");
    expect(result.authorizedSurfaces).toContain("components/dashboard/revenue-bottleneck-resolution-summary.tsx");
  });

  it("smoke-tests read-only UI scope readiness", () => {
    const result = createR89RevenueBottleneckResolutionReadonlyUiScopeContract(reviewedInput);
    expect(result.status).toBe("revenue_bottleneck_resolution_ui_scope_ready");
  ...
  it("keeps wording advisory and non-executing", () => {
    const result = createR89RevenueBottleneckResolutionReadonlyUiScopeContract(reviewedInput);
    expect(result.wording.bottleneckDiagnosis).toMatch(/advisory visibility only/i);
    expect(result.wording.remediation).toMatch(/Human remediation review required/i);
  ...
  it("pressure-tests forbidden UI controls", () => {
    ...
    expect(result.status).toBe("revenue_bottleneck_resolution_ui_scope_blocked");
  ...
  it("summarizes read-only scope", () => {
    const result = createR89RevenueBottleneckResolutionReadonlyUiScopeContract(reviewedInput);
    expect(summarizeR89RevenueBottleneckResolutionReadonlyUiScope(result)).toMatch(/read-only advisory wording/i);
  });
});
