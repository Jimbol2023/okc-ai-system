...
export async function POST(request: Request) {
  ...
  const result = await runTwilioDiagnosticPreviewRead({ actor: { tenantId: actor.tenantId, actorId: actor.actorId }, confirmation: input.confirmation ?? "", capability: input.capability, nonce: input.nonce, phoneNumberSid: input.phoneNumberSid });
