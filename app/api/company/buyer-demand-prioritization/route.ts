...
export async function GET(request: Request) {
  ...
  try {
    const [snapshots, buyerDemandSignals] = await Promise.all([
      getLatestTenantBusinessSnapshots(actor.tenantId, 200),
    ...
    const prioritization = createBuyerDemandOpportunityPrioritization({ tenantId: actor.tenantId, certification, buyerDemandSignals });

    return NextResponse.json({ ok: true, ...prioritization });
  } catch (error) {
