...
import { rollbackSearchConsolePreview } from "@/lib/ueip-preview-pilot";
...
export async function POST(request: Request) {
  ...
  return NextResponse.json(await rollbackSearchConsolePreview({ actor: { tenantId: auth.tenantId, actorId: auth.actorId }, confirmation: body.confirmation ?? "", action: body.action as "drill_disable" | "drill_restore" | "emergency_disable" }));
}
