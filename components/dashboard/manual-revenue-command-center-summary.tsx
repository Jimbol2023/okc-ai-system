import { r87AdvisoryRevenueCommandCenterCategories } from "@/lib/r87-manual-revenue-command-center-scope-contract";
import { r87BlockedDriftTransitions } from "@/lib/r87-manual-revenue-command-center-drift-risk-audit";
import { r87ReadonlyUiWording } from "@/lib/r87-manual-revenue-command-center-readonly-ui-scope-contract";

type RevenueCommandCenterSignal = {
  title: string;
  status: string;
  detail: string;
};
...
const revenueSignals: RevenueCommandCenterSignal[] = [
  {
    title: "Executive revenue visibility",
    status: r87ReadonlyUiWording.manualReview,
    detail: "Executive revenue labels help humans inspect opportunity timing, incomplete signals, blocked paths, and low-confidence high-opportunity areas without changing records.",
  },
  {
    title: "Throughput oversight",
    status: r87ReadonlyUiWording.throughput,
    detail: "Throughput review may show where acquisition workflows affect deal-flow timing. Runtime jobs, polling, and automation remain blocked.",
  },
  {
    title: "Bottleneck coordination",
    status: r87ReadonlyUiWording.bottleneck,
    detail: "Bottleneck visibility can support manual coordination across delayed deal-flow, blocked revenue paths, and incomplete revenue visibility without provider behavior.",
  },
  {
    title: "Revenue delay risk",
    status: r87ReadonlyUiWording.revenueDelay,
  ...
  },
  {
    ...
    status: r87ReadonlyUiWording.assignmentClosingReview,
    detail: "Human review required before any operational action.",
  },
  {
    title: "Governance boundary",
    status: r87ReadonlyUiWording.governance,
    detail: `${r87ReadonlyUiWording.noExecution} ${r87ReadonlyUiWording.noProvider} ${r87ReadonlyUiWording.noContact}`,
  },
];
...
export function ManualRevenueCommandCenterSummary() {
  return (
    <section
      aria-labelledby="manual-revenue-command-center-heading"
      aria-describedby="manual-revenue-command-center-summary"
    ...
    >
      <div className="flex min-w-0 flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0 space-y-2">
          <p className="break-words text-sm font-semibold uppercase tracking-[0.16em] text-muted">
            Manual revenue command center
          </p>
          <h2 id="manual-revenue-command-center-heading" className="break-words text-xl font-semibold text-primary">
            Read-only revenue command center intelligence
          </h2>
          <p id="manual-revenue-command-center-summary" className="max-w-3xl break-words text-sm leading-6 text-muted">
            {r87ReadonlyUiWording.revenueCommandCenter} It helps operators review executive revenue visibility,
            throughput review needs, bottleneck coordination, delayed deal-flow, assignment review, closing review,
            incomplete revenue visibility, human escalation, and governance review needs. No execution, provider activation, outreach, automation,
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="min-w-0 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-blue-950">
          <h3 className="break-words font-semibold text-blue-950">Advisory revenue command center categories</h3>
          <div className="mt-3 flex max-w-full flex-wrap gap-2">
            {r87AdvisoryRevenueCommandCenterCategories.map((category) => (
        ...
        <div className="min-w-0 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-blue-950">
          <h3 className="break-words font-semibold text-blue-950">Revenue command drift boundary</h3>
          <ul className="mt-2 list-disc space-y-1 pl-5">
            {r87BlockedDriftTransitions.slice(0, 7).map((transition) => (
